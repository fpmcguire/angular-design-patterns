...
test.describe('SOLID Principles Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/solid');
    await dismissCookieBannerIfVisible(page);
  });

  test('should display SOLID page with title and intro', async ({ page }) => {
    // Verify page title
    const title = page.getByTestId('patterns-title');
    await expect(title).toBeVisible();
    await expect(title).toHaveText('S.O.L.I.D. Principles');

    // Verify intro text
    const intro = page.getByTestId('patterns-intro');
    await expect(intro).toBeVisible();
    await expect(intro).toContainText('object-oriented design');
  });
  ...
  test('should navigate to principle detail when card clicked', async ({ page }) => {
    ...
    await expect(page.getByTestId('pattern-detail-back-link')).toHaveAttribute('href', /\/solid$/);
  });
