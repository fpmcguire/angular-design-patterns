...
test.describe('Frontend Architecture Principles Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/frontend-architecture');
    await dismissCookieBannerIfVisible(page);
  });

  test('should display frontend architecture page with title and intro', async ({ page }) => {
    // Verify page title
    const title = page.getByTestId('patterns-title');
    await expect(title).toBeVisible();
    await expect(title).toHaveText('Frontend Architecture Principles');

    // Verify intro text
    const intro = page.getByTestId('patterns-intro');
    await expect(intro).toBeVisible();
    await expect(intro).toContainText('frontend');
  });
