...
test.describe('Reactive Principles Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/reactive');
    await dismissCookieBannerIfVisible(page);
  });

  test('should display reactive principles page with title and intro', async ({ page }) => {
    // Verify page title
    const title = page.getByTestId('patterns-title');
    await expect(title).toBeVisible();
    await expect(title).toHaveText('Reactive Principles');

    // Verify intro text
    const intro = page.getByTestId('patterns-intro');
    await expect(intro).toBeVisible();
    await expect(intro).toContainText('reactive');
  });
