...
test.describe('Classic Patterns Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/classic');
    await dismissCookieBannerIfVisible(page);
  });

  test('should display classic patterns page with title and intro', async ({ page }) => {
    // Verify page title
    const title = page.getByTestId('patterns-title');
    await expect(title).toBeVisible();
    await expect(title).toHaveText('Classic Design Patterns (GoF)');

    // Verify intro text
    const intro = page.getByTestId('patterns-intro');
    await expect(intro).toBeVisible();
    await expect(intro).toContainText('design patterns');
  });
  ...
  test('should display pattern cards', async ({ page }) => {
    ...
    // Verify pattern cards are visible
    const cards = page.getByTestId('pattern-card');
    await expect(cards.first()).toBeVisible();

    // Verify there are multiple pattern cards
    const count = await cards.count();
    expect(count).toBeGreaterThan(0);
  });

  test('should display pattern card details', async ({ page }) => {
    await page.waitForLoadState('networkidle');
  ...
  test('should filter patterns by category', async ({ page }) => {
    ...
    if (hasFilters) {
      ...
      // Should have some patterns but likely fewer than initial
      ...
      // Verify we're back to showing all patterns
  ...
  test('should navigate to pattern detail when card clicked', async ({ page }) => {
    await page.waitForLoadState('networkidle');

    // Get the first pattern card and its title
    const firstCard = page.getByTestId('pattern-card').first();
    const patternTitle = await firstCard.getByTestId('pattern-card-title').textContent();
