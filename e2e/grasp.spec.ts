...
test.describe('GRASP Principles Page', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/grasp');
    await dismissCookieBannerIfVisible(page);
  });

  test('should display GRASP page with title and intro', async ({ page }) => {
    // Verify page title
    const title = page.getByTestId('patterns-title');
    await expect(title).toBeVisible();
    await expect(title).toHaveText('GRASP Principles');

    // Verify intro text
    const intro = page.getByTestId('patterns-intro');
    await expect(intro).toBeVisible();
    await expect(intro).toContainText('General Responsibility Assignment');
  });
  ...
  test('should navigate to principle detail when card clicked', async ({ page }) => {
    ...
    await expect(page.getByTestId('pattern-detail-back-link')).toHaveAttribute('href', /\/grasp$/);
  });
